...
- create-agent prompt pack (every turn): `reasons.md` + derived netlist + library catalog (ids + pins) + last 5 history lines (what+why) + sheet PNGs + user text/files. no ERC, no SVG markup, no full JSON dump. open nets during creation are normal. LLM never gets schematic *only* as an image. returns diff + message OR a question if not 100% sure; applied diff re-renders and rewrites `render/`
